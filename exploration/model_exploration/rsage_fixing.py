...
def hits_at_k(model,dataset,k,args,return_indices=False) -> dict:
  hits = {}
  with torch.no_grad():
    preds = model(dataset)
    for key,pred in preds.items():
...
#%%
result = optimization_iteration(Hete,train_data,val_data,train_mode,"gda_only","dotprod",5,"random",args,"figures/")
#%%

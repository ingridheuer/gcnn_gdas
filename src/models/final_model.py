...
class Encoder(torch.nn.Module):
    def __init__(self, metadata):
        super().__init__()
        output_dim = 64
        self.graph_layer_1 = to_hetero(
            GraphBlock(-1, output_dim, 0.4, False), metadata, aggr="mean")
        self.graph_layer_2 = to_hetero(GraphBlock(
            output_dim, output_dim, 0.1, True), metadata, aggr="mean")
        self.graph_layer_3 = to_hetero(GraphBlock(
            output_dim, output_dim, 0.1, True), metadata, aggr="mean")
        self.post_mlp = to_hetero(PostProcessMLP(
            output_dim, output_dim, 0.1), metadata)

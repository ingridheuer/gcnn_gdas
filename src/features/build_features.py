...
def fill_missing_with_mean(feature_matrix,node_type,node_data=tensor_df):
    ...
    assert len(full_feature_df) == node_count, f"Lenght of feature tensor {len(full_feature_df)} does not match number of nodes {node_count}"
...
norm_df = pd.DataFrame(norm_values, index=genes_features.index, columns=genes_features.columns)
